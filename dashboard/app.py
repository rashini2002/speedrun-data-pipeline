...
# Exclude 'unknown' from the map itself (can't plot a country you don't have)
geo_df_known = geo_df[geo_df["country_code"] != "unknown"]
...
if geo_df_known.empty:
    ...
    fig_map = px.choropleth(
        ...
        locationmode="ISO-3",  # see note below on 2-letter vs 3-letter codes
...
unknown_pct = (geo_df[geo_df["country_code"] == "unknown"]["distinct_runners"].sum() 
               / geo_df["distinct_runners"].sum() * 100) if not geo_df.empty else 0
st.caption(f"Note: {unknown_pct:.1f}% of runners have no resolved country (see project notes on player enrichment scope)")
